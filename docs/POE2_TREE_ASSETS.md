...
## Verbindungsebenen 5D.4.1

`line.json` enthält allgemeine aktive, normale und intermediäre Linien-/Orbit-Sprites sowie zwei Dekoelemente. `data.json.edges` verweist diese Sprites jedoch nicht pro Kante und liefert keinen eigenen Glow- oder Dekorationsstatus. Eine solche Zuordnung wird daher nicht erfunden. Der einzige eindeutige Sichtbarkeitshinweis ist `nodes.*.hideConnection`: zwölf Smith-of-Kitava-Spezialknoten markieren damit ihre Verbindung zu Smith’s Masterwork als im Ruhezustand verborgen. Der Assetimport, die Atlanten und sämtliche Knotenmotive bleiben durch 5D.4.1 unverändert.
