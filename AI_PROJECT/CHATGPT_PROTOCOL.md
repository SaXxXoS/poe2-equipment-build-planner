...
- Die vorgesehene öffentliche URL lautet `https://saxxxos.github.io/poe2-equipment-build-planner/`, ist aber noch nicht als erreichbar bestätigt. Actions-Lauf `29735420251` akzeptierte den Workflow, installierte mit `npm ci` und baute erfolgreich; `Configure GitHub Pages` scheiterte danach mit `Get Pages site failed`/`Not Found`, weil Pages im Repository noch nicht aktiviert und auf GitHub Actions gestellt ist. Der Deploy-Job wurde übersprungen.
- Einmaliger externer Schritt mit Repository-Administratorrechten: **Settings → Pages → Build and deployment → Source → GitHub Actions**; anschließend den Workflow **Deploy GitHub Pages** erneut starten oder einen neuen Push auf `main` auslösen. Die offizielle `enablement`-Option wird nicht verwendet, weil sie einen zusätzlichen Admin-/PAT-Token statt des minimal berechtigten `GITHUB_TOKEN` voraussetzt.
...
- Die öffentliche Ziel-URL lieferte am 20. Juli 2026 weiterhin GitHubs Seite `404 – There isn't a GitHub Pages site here`. Daher konnten die funktionalen Smoke-Tests nicht auf der echten Pages-Domain wiederholt werden. Nicht auf einem physischen Touchgerät getestet; Touch-/Pan-Verhalten wurde nur indirekt über Pointer-Events und Mobilbreite abgedeckt.
