...
export function normalizeTree(raw, metadata) {
  ...
  const connections = raw.edges.filter(edge => nodeIds.has(String(edge.from)) && nodeIds.has(String(edge.to)) && String(edge.from) !== String(edge.to)).map((edge, sourceIndex) => { const [fromNodeId, toNodeId] = sortedIds([edge.from, edge.to]),hideInDefaultState=raw.nodes[fromNodeId]?.hideConnection === true || raw.nodes[toNodeId]?.hideConnection === true; return { id: `${fromNodeId}:${toNodeId}`, fromNodeId, toNodeId, connectionType: 'passive-tree', directed: false, ...(hideInDefaultState?{hideInDefaultState:true}:{}), sourceReference: `edges.${sourceIndex}` } }).sort((a, b) => a.id.localeCompare(b.id, 'en', { numeric: true }))
