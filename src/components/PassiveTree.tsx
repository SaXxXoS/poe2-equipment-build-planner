...
const NodeLayer = memo(function NodeLayer({ tree, selectedId, filter, onSelect }: { tree: PassiveTreeViewModel; selectedId: string | null; filter: TreeDisplayFilter; onSelect: (node: TreeNodeViewModel) => void }) {
  return <g className="tree-nodes">{tree.nodes.map(node => { const dimmed = !nodeMatchesFilter(node, filter); return <circle key={node.id} cx={node.x} cy={node.y} r={TREE_NODE_RADIUS[node.nodeType]} className={`tree-node tree-node-${node.nodeType}${selectedId === node.id ? ' selected' : ''}${dimmed ? ' dimmed' : ''}`} role="button" aria-label={`${node.displayName}, ${node.nodeType}, ID ${node.id}`} tabIndex={0} onPointerDown={event => event.stopPropagation()} onClick={() => onSelect(node)} onKeyDown={event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); onSelect(node) } }}/> })}</g>
})
...
export function PassiveTree({ loader = loadPoe2TreeViewModel }: { loader?: () => Promise<PassiveTreeViewModel> }) {
  ...
  return <PassiveTreeContent tree={tree} full={full} onToggleFull={() => setFull(value => !value)} performanceMetrics={{ ...treeLoadMetrics, firstRenderMs }}/>
}
...
export function PassiveTreeContent({ tree, full = false, onToggleFull = () => undefined, performanceMetrics }: { tree: PassiveTreeViewModel; full?: boolean; onToggleFull?: () => void; performanceMetrics?: { dataLoadMs: number; adapterMs: number; totalMs: number; firstRenderMs: number } }) {
  ...
  return <section className={full ? 'tree-section tree-full' : 'tree-section'} style={{ minWidth: 0, overflowX: 'hidden' }}><div className="row"><div><h2>5. Offizieller PoE2-Passivbaum</h2><p className="muted">GGG-Export · Release {tree.sourceVersion} · {tree.nodeCount.toLocaleString('de-DE')} Knoten · englische Originaltexte</p></div><button className="secondary" onClick={onToggleFull}>{full ? 'Verkleinern' : 'Vergrößern'}</button></div>
    ...
    <div className="tree-tools"><label>Knotensuche<input value={query} onChange={event => setQuery(event.target.value)} placeholder="Englischer Name, Stat oder ID"/></label><label>Darstellungsfilter<select value={filter} onChange={event => setFilter(event.target.value as TreeDisplayFilter)}>{FILTERS.map(value => <option key={value.id} value={value.id}>{value.label}</option>)}</select></label><label>Klassenstart<select defaultValue="" onChange={event => { orient(event.target.value); event.target.value = '' }}><option value="">Klasse auswählen</option>{tree.classes.map(value => <option key={`${value.displayName}-${value.nodeId}`} value={value.nodeId}>{value.displayName}</option>)}</select></label><label>Aszendenzstart<select defaultValue="" onChange={event => { orient(event.target.value); event.target.value = '' }}><option value="">Aszendenz auswählen</option>{tree.ascendancies.map(value => <option key={`${value.displayName}-${value.nodeId}`} value={value.nodeId}>{value.displayName}</option>)}</select></label></div>
    {query.trim() && <div className="tree-search-results" aria-label="Suchergebnisse" style={{ maxWidth: '100%', minWidth: 0 }}><span>{results.length} Treffer angezeigt</span>{results.map(node => <button key={node.id} className="secondary" onClick={() => focusNode(node)}>{node.displayName} <small>#{node.id}</small></button>)}</div>}
