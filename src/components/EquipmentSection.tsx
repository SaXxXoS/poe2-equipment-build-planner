...
const visibleSlotName = (id: string) => id.includes('weapon') ? (id.endsWith('left') ? 'Waffe L' : 'Waffe R') : id === 'slot-ring-1' ? 'R1' : id === 'slot-ring-2' ? 'R2' : id === 'slot-amulet' ? 'A' : id === 'slot-life-flask' ? 'Leben' : id === 'slot-mana-flask' ? 'Mana' : id === 'slot-jewel-1' ? 'Juwel 1' : id === 'slot-jewel-2' ? 'Juwel 2' : slotName(id)
...
const quickSlots = ['slot-life-flask','slot-charm-1','slot-charm-2','slot-mana-flask']
const jewelSlots = ['slot-jewel-1','slot-jewel-2']
export function EquipmentSection({ entries, setEntries }: { entries: EquipmentEntry[]; setEntries: (values: EquipmentEntry[]) => void }) {
  ...
  return <section id="equipment"><h2>2. Ausrüstung</h2><p className="muted">Tippe einen Platz an, um deinen Gegenstand einzutragen.</p>
    ...
    <div className="equipment-jewels"><h3>Juwelen</h3><div>{jewelSlots.map(id => renderSlot(id, true))}</div></div>
    </div>
