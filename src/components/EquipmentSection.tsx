...
const evidenceName:Record<NonNullable<EquipmentSlotSuggestion['semanticEvidence']>,string>={
  'structured-exact':'Direkt aus strukturierten Daten',
  'structured-derived':'Sicher aus strukturierten Daten abgeleitet',
  'text-pattern-exact':'Aus einem eindeutig geprüften Beschreibungsmuster',
  'text-pattern-ambiguous':'Nicht produktiv verwendbare Mehrdeutigkeit',
...
export function EquipmentSection({ entries, setEntries, suggestions=[] }: { entries: EquipmentEntry[]; setEntries: (values: EquipmentEntry[]) => void; suggestions?:EquipmentSlotSuggestion[] }) {
  ...
  const suggestedEvidenceLines=suggestedUnique?(activeSuggestion?.evidenceLineIds??[])
    .map(id=>[...suggestedUnique.implicits,...suggestedUnique.modifiers].find(line=>line.id===id)?.text)
    .filter((value):value is string=>Boolean(value)):[]
