...
export function SkillsSection({ setups, onChange, onRecommendSupports }: { setups: SkillSetup[]; onChange: (values: SkillSetup[]) => void; onRecommendSupports?: (setupId: string) => void }) {
  ...
  return <section id="skills"><div className="section-heading"><div><h2>3. Fertigkeiten und Unterstützungen</h2><p className="muted">Sechs leere Startplätze; weitere Plätze sind optional.</p></div><button aria-label="Fertigkeitsslot hinzufügen" onClick={() => onChange([...visible, emptySkillSetup(visible.length)])}>＋ Slot</button></div>
    <div className="skills-grid">{visible.map((setup, index) => {
      ...
      if (!skill) return <article className="skill-card empty-skill-card" key={setup.id}><div className="skill-card-head"><span className="skill-art" aria-hidden="true">＋</span><div><h3>Fertigkeit {index + 1}</h3><small>Keine Fertigkeit ausgewählt</small></div>{index >= 6 && <button aria-label="Fertigkeitsslot entfernen" onClick={() => onChange(visible.filter(value => value.id !== setup.id))}>−</button>}</div>
        <label>Fertigkeit suchen<input type="search" value={query} placeholder="Fertigkeit suchen" onChange={event => setSearches({ ...searches, [setup.id]: event.target.value })}/></label>
        <label>Fertigkeit<select aria-label={`Fertigkeit ${index + 1} auswählen`} value="" onChange={event => event.target.value && update(setup.id, { skillId: event.target.value, origin: 'manual', supportGemIds: [] })}><option value="">Fertigkeit auswählen</option>{filtered.map(item => <option value={item.id} key={item.id}>{item.displayNameDe}{item.nameEn && item.nameEn !== item.displayNameDe ? ` (${item.nameEn})` : ''}</option>)}</select></label>
      </article>
      return <article className="skill-card populated-skill-card" key={setup.id}><div className="skill-card-head"><span className="skill-art" aria-hidden="true">{skill.displayNameDe.slice(0, 2)}</span><div><h3>{skill.displayNameDe}</h3><small>{originLabels[setup.origin ?? 'manual']}</small></div>{index >= 6 && !setup.locked && <button aria-label="Fertigkeitsslot entfernen" onClick={() => onChange(visible.filter(value => value.id !== setup.id))}>−</button>}</div>
        ...
        <div className="support-editor"><div className="row"><b>Unterstützungen</b><button onClick={() => onRecommendSupports?.(setup.id)}>Beste vorschlagen</button></div>{setup.supportGemIds.map(id => { const support = buildAssistantCandidates.supports.find(item => item.id === id); return <span className="support-chip" key={id}><span className="support-art" aria-hidden="true">{support?.displayNameDe.slice(0, 2) ?? '?'}</span><span>{support?.displayNameDe ?? id}</span><button aria-label={`${support?.displayNameDe ?? id} entfernen`} onClick={() => update(setup.id, { supportGemIds: setup.supportGemIds.filter(value => value !== id) })}>−</button></span> })}<select aria-label="Unterstützung hinzufügen" value="" onChange={event => event.target.value && update(setup.id, { supportGemIds: [...new Set([...setup.supportGemIds, event.target.value])] })}><option value="">＋ Unterstützung hinzufügen</option>{buildAssistantCandidates.supports.filter(item => !setup.supportGemIds.includes(item.id)).map(item => <option key={item.id} value={item.id}>{item.displayNameDe}</option>)}</select></div>
      </article>
    })}</div><p className="muted">Automatische Empfehlungen erscheinen erst nach einer Build-Auswertung oder einer ausdrücklichen Aktion.</p>
  </section>
}
