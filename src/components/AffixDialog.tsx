...
export function AffixDialog({ entry, slotName, onSave, onClose }: { entry: EquipmentEntry; slotName: string; onSave: (entry: EquipmentEntry) => void; onClose: () => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={event => event.target === event.currentTarget && onClose()}><div className="modal item-editor" role="dialog" aria-modal="true">
    ...
    <div className="dialog-scroll">
      ...
      {step === 'editor' && !picker && <>{rarity === 'unique' ? <><label>Unique suchen<input autoFocus type="search" value={uniqueSearch} onChange={event => setUniqueSearch(event.target.value)} placeholder="Name oder Basistyp"/></label><label>Unique<select className="affix-list" size={7} value={uniqueItemId} onChange={event => { setUniqueItemId(event.target.value); setUniqueVariantId('') }}><option value="">Bitte wählen</option>{uniqueOptions.map(item => <option value={item.id} key={item.id}>{item.name} · {item.baseDisplayName}</option>)}</select></label>{chosenUnique && <div className="unique-choice-summary"><b>{chosenUnique.name}</b><span>{chosenUnique.baseDisplayName}</span>{chosenUnique.variants.length > 1 && <label>Variante<select value={uniqueVariantId} onChange={event => setUniqueVariantId(event.target.value)}><option value="">Variante nicht festgelegt</option>{chosenUnique.variants.map(variant => <option value={variant.id} key={variant.id}>{variant.text} · {variant.currentOrLegacy === 'legacy' ? 'Legacy' : variant.currentOrLegacy === 'current' ? 'Aktuell' : 'Status unbekannt'}</option>)}</select></label>}<small>PoB2-Planerdaten; normale Affixe bleiben getrennt.</small></div>}</> : <>
    ...
    </div>{step === 'editor' && !picker && <div className="dialog-actions"><button className="secondary" onClick={onClose}>Abbrechen</button><button disabled={rarity === 'unique' ? !uniqueItemId : !itemClassId || baseItems.length > 0 && !baseItemId} onClick={save}>Speichern</button></div>}
  </div></div>
}
