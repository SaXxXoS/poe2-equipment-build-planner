...
import type { AppliedModifier, EquipmentEntry, EquipmentWeaponStats, ItemPropertyKind, ItemRarity } from '../domain'
...
export function WeaponStatsFields({value,onChange}:{value:EquipmentWeaponStats;onChange:(value:EquipmentWeaponStats)=>void}) {
  ...
  return <details open className="weapon-base-values"><summary>Waffen-Grundwerte</summary><p className="muted">Trage die endgültigen Werte aus der Gegenstandsanzeige ein. Minimum und Maximum sind getrennt editierbar.</p><div className="weapon-base-value-grid">{range('physicalDamage','Physischer Schaden')}{range('fireDamage','Feuerschaden')}{range('coldDamage','Kälteschaden')}{range('lightningDamage','Blitzschaden')}{range('chaosDamage','Chaosschaden')}<div className="weapon-single-values">{number('criticalHitChance','Kritische Trefferchance (%)','0.01')}{number('attacksPerSecond','Angriffe pro Sekunde','0.01')}</div></div>{value.unresolvedElementalDamage?.length?<p className="warning">Im Bild wurden {value.unresolvedElementalDamage.length} elementare Schadensbereiche erkannt, deren Schadensart anhand des OCR-Texts nicht sicher bestimmt werden konnte. Bitte trage sie oben bei Feuer, Kälte oder Blitz ein.</p>:null}</details>
}
...
export function AffixDialog({ entry, slotName, onSave, onClose }: { entry: EquipmentEntry; slotName: string; onSave: (entry: EquipmentEntry) => void; onClose: () => void }) {
  ...
  const [defencesSource,setDefencesSource]=useState(entry.defencesSource??(entry.defences?'observed-final' as const:defenceBaseValueById.has(entry.itemDefinitionId??'')?'pinned-base' as const:undefined))
  const [weaponStats,setWeaponStats]=useState<EquipmentWeaponStats>(entry.weaponStats??{})
  const [weaponStatsSource,setWeaponStatsSource]=useState(entry.weaponStatsSource??(entry.weaponStats?'observed-final' as const:weaponBaseValueById.has(entry.itemDefinitionId??'')?'pinned-base' as const:undefined))
  ...
  const supportsDefences=itemSupportsDefenceValues(itemClassId)
  const baseItems = baseItemsFor(itemClassId)
  const weaponBases = entry.slotId.includes('weapon') ? weaponBaseValuesFor(itemClassId) : []
  const defenceBases = !entry.slotId.includes('weapon') ? defenceBaseValuesFor(itemClassId) : []
  const utilityBases = utilityBaseValuesFor(itemClassId)
  const chosenUnique = localizedPob2UniquesDe.find(item => item.id === uniqueItemId)
  const showsDefenceFields = rarity === 'unique' ? itemSupportsDefenceValues(undefined, chosenUnique?.itemCategory ?? '') : supportsDefences
  const showsWeaponFields=rarity==='unique'
    ? entry.slotId.includes('weapon')&&(chosenUnique?.itemCategory==='weapon'||chosenUnique?.itemCategory==='offhand')
    : weaponBases.length>0
  ...
  const orderedAdded=[...added].sort((left,right)=>(left.sourceOrder??Number.MAX_SAFE_INTEGER)-(right.sourceOrder??Number.MAX_SAFE_INTEGER)||['implicit','prefix','suffix'].indexOf(left.affixSide??'')-['implicit','prefix','suffix'].indexOf(right.affixSide??''))
  ...
  function save() {
    const defences=!showsDefenceFields||armour===undefined&&evasion===undefined&&energyShield===undefined?undefined:{armour,evasion,energyShield}
    const savedWeaponStats=showsWeaponFields&&Object.values(weaponStats).some(value=>value!==undefined)?weaponStats:undefined
    onSave(rarity === 'unique' ? { ...entry, rarity, uniqueItemId, uniqueVariantId: uniqueVariantId || undefined, observedUniqueLines:observedItemLines, observedItemLines, observedImplicitLines, properties, sockets:undefined, modifierValues: [], itemClassId: undefined, itemDefinitionId: undefined, baseDisplayName: undefined, itemLevel, quality, defences, defencesSource:defences?defencesSource:undefined, weaponStats:savedWeaponStats, weaponStatsSource:savedWeaponStats?weaponStatsSource:undefined } : { ...entry, rarity, uniqueItemId: undefined, uniqueVariantId: undefined, observedUniqueLines:undefined, observedItemLines, observedImplicitLines, properties, sockets:undefined, itemClassId, itemDefinitionId: baseItemId || undefined, baseDisplayName: baseDisplayName.trim() || undefined, itemLevel, quality, defences, defencesSource:defences?defencesSource:undefined, weaponStats:savedWeaponStats, weaponStatsSource:savedWeaponStats?weaponStatsSource:undefined, modifierValues: added })
    onClose()
  }
  return <div className="modal-backdrop" onMouseDown={event => event.target === event.currentTarget && onClose()}><div className={`modal item-editor ${entry.slotId.includes('weapon')?'weapon-item-editor':''}`} role="dialog" aria-modal="true">
    ...
    <div className="dialog-scroll">
      ...
      {step === 'editor' && !picker && <>{rarity === 'unique' ? <><label>Unique suchen<input autoFocus type="search" value={uniqueSearch} onChange={event => setUniqueSearch(event.target.value)} placeholder="Name oder Basistyp"/></label><label>Unique<select className="affix-list" size={7} value={uniqueItemId} onChange={event => { setUniqueItemId(event.target.value); setUniqueVariantId('') }}><option value="">Bitte wählen</option>{uniqueOptions.map(item => <option value={item.id} key={item.id}>{item.name} · {item.baseDisplayName}</option>)}</select></label>{chosenUnique && <div className="unique-choice-summary">
        ...
        <div className="form-grid compact"><label>Qualität (%)<input type="number" min="0" value={quality ?? ''} onChange={event=>setQuality(event.target.value===''?undefined:Number(event.target.value))}/></label>{showsDefenceFields&&<><label>Rüstung<input type="number" min="0" value={armour ?? ''} onChange={event=>{setArmour(event.target.value===''?undefined:Number(event.target.value));setDefencesSource('observed-final')}}/></label><label>Ausweichwert<input type="number" min="0" value={evasion ?? ''} onChange={event=>{setEvasion(event.target.value===''?undefined:Number(event.target.value));setDefencesSource('observed-final')}}/></label><label>Energieschild<input type="number" min="0" value={energyShield ?? ''} onChange={event=>{setEnergyShield(event.target.value===''?undefined:Number(event.target.value));setDefencesSource('observed-final')}}/></label></>}</div>
        {showsWeaponFields&&<WeaponStatsFields value={weaponStats} onChange={value=>{setWeaponStats(value);setWeaponStatsSource('observed-final')}}/>}
        <small>PoB2-Planerdaten; normale Affixe bleiben getrennt.</small>
      </div>}</> : <>
        <details open><summary>Grunddaten · {rarityText[rarity]}</summary><label>Itemklasse<select value={itemClassId} onChange={event => { setItemClassId(event.target.value); setBaseItemId(''); setBaseDisplayName(''); setWeaponStats({}); setWeaponStatsSource(undefined); setArmour(undefined); setEvasion(undefined); setEnergyShield(undefined); setDefencesSource(undefined) }}>{classes.map(value => <option value={value.itemClassId} key={value.itemClassId}>{value.technicalName}</option>)}</select></label>{weaponBases.length > 0 ? <label>Basistyp<select value={baseItemId} onChange={event => { const id=event.target.value; const base=weaponBaseValueById.get(id); setBaseItemId(id); setBaseDisplayName(base?.nameEn??''); if(base){setWeaponStats(weaponStatsFromBase(base));setWeaponStatsSource('pinned-base')} }}><option value="">Bitte wählen</option>{weaponBases.map(value => <option key={value.id} value={value.id}>{weaponBaseDisplayName(value)}</option>)}</select><small>Die gepinnten Grundwerte werden automatisch eingesetzt und können durch die tatsächlichen Tooltip-Endwerte ersetzt werden.</small></label> : defenceBases.length > 0 ? <label>Basistyp<select value={baseItemId} onChange={event => { const id=event.target.value; const base=defenceBaseValueById.get(id); setBaseItemId(id); setBaseDisplayName(base?.nameEn??''); if(base){setArmour(base.defences.armour??undefined);setEvasion(base.defences.evasion??undefined);setEnergyShield(base.defences.energyShield??undefined);setDefencesSource('pinned-base')} }}><option value="">Bitte wählen</option>{defenceBases.map(value => <option key={value.id} value={value.id}>{defenceBaseDisplayName(value)}</option>)}</select><small>Die gepinnten Grundwerte werden automatisch eingesetzt. Trägst du Tooltip-Endwerte ein, werden lokale Affixe nicht erneut addiert.</small></label> : utilityBases.length > 0 ? <label>Basistyp<select value={baseItemId} onChange={event => { const id=event.target.value; const base=utilityBaseValueById.get(id); setBaseItemId(id); setBaseDisplayName(base?.nameEn??'') }}><option value="">Bitte wählen</option>{utilityBases.map(value => <option key={value.id} value={value.id}>{utilityBaseDisplayName(value)}</option>)}</select>{utilityBaseValueById.get(baseItemId)?.implicit&&<small>Implizit: {utilityBaseValueById.get(baseItemId)?.implicit} (sichtbare PoB2-Quellzeile; technische Wirkung nur bei gesonderter Belegung)</small>}</label> : baseItems.length > 0 ? <label>Basistyp<select value={baseItemId} onChange={event => setBaseItemId(event.target.value)}><option value="">Bitte wählen</option>{baseItems.map(value => <option key={value.baseItemId} value={value.baseItemId}>{value.baseItemId}</option>)}</select></label> : <label>Basistyp (Anzeige)<input value={baseDisplayName} onChange={event => setBaseDisplayName(event.target.value)} placeholder="z. B. Experten-Kettenhaube"/></label>}<label>Item-Level (nur zum technischen Filtern)<input type="number" min="1" value={itemLevel ?? ''} onChange={event => setItemLevel(event.target.value ? Number(event.target.value) : undefined)}/></label><div className="form-grid compact"><label>Qualität (%)<input type="number" min="0" value={quality ?? ''} onChange={event=>setQuality(event.target.value===''?undefined:Number(event.target.value))}/></label>{showsDefenceFields&&<><label>Rüstung<input type="number" min="0" value={armour ?? ''} onChange={event=>{setArmour(event.target.value===''?undefined:Number(event.target.value));setDefencesSource('observed-final')}}/></label><label>Ausweichwert<input type="number" min="0" value={evasion ?? ''} onChange={event=>{setEvasion(event.target.value===''?undefined:Number(event.target.value));setDefencesSource('observed-final')}}/></label><label>Energieschild<input type="number" min="0" value={energyShield ?? ''} onChange={event=>{setEnergyShield(event.target.value===''?undefined:Number(event.target.value));setDefencesSource('observed-final')}}/></label></>}</div>{showsDefenceFields&&<p className="muted">{defencesSource==='pinned-base'?'Gepinnte Basiswerte; lokale Affixe dürfen genau einmal darauf wirken.':'Diese endgültigen Gegenstandswerte werden getrennt von den Affixen gespeichert und defensiv ausgewertet.'}</p>}</details>
        {showsWeaponFields&&<WeaponStatsFields value={weaponStats} onChange={value=>{setWeaponStats(value);setWeaponStatsSource('observed-final')}}/>}
      </>}
      <details open><summary>Eigenschaften · {added.length+properties.length}</summary>
        {rarity!=='unique'&&<>{orderedAdded.filter((value):value is AppliedModifier&{affixSide:'prefix'|'suffix'|'implicit'}=>value.affixSide==='prefix'||value.affixSide==='suffix'||value.affixSide==='implicit').map(value=><AppliedSlot key={value.id} side={value.affixSide} index={added.filter(candidate=>candidate.affixSide===value.affixSide).findIndex(candidate=>candidate.id===value.id)} value={value} onChoose={()=>setPicker({side:value.affixSide,index:Number(value.id.split(':').at(-1)??1)-1})} onRemove={()=>setAdded(current=>current.filter(candidate=>candidate.id!==value.id))}/>)}
        ...
        <p className="muted">Nur technisch zugeordnete Eigenschaften mit den erkannten tatsächlichen Werten fließen in die Buildberechnung ein. Unbekannter OCR-Text wird nicht als scheinbar wirksame Eigenschaft gespeichert.</p>
      </details></>}
    ...
    </div>{step === 'editor' && !picker && <div className="dialog-actions"><button className="secondary" onClick={onClose}>Abbrechen</button><button disabled={!weaponStatsAreValid(weaponStats)||(rarity === 'unique' ? !uniqueItemId || Boolean(chosenUnique && chosenUnique.variants.length > 1 && !uniqueVariantId) : !itemClassId || (baseItems.length > 0 || weaponBases.length > 0 || defenceBases.length > 0 || utilityBases.length > 0) && !baseItemId)} onClick={save}>Speichern</button></div>}
  </div></div>
}
