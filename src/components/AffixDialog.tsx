...
export function AffixDialog({ entry, slotName, onSave, onClose }: { entry: EquipmentEntry; slotName: string; onSave: (entry: EquipmentEntry) => void; onClose: () => void }) {
  ...
  const showsWeaponFields=entry.slotId.includes('weapon')&&(rarity!=='unique'||chosenUnique?.itemCategory==='weapon'||chosenUnique?.itemCategory==='offhand')
  ...
  function applyOcr(result:ItemOcrResult,selectedIds:Set<string>){
    ...
    setItemLevel(result.itemLevel);setQuality(result.quality);setArmour(ocrSupportsDefences?result.defences?.armour:undefined);setEvasion(ocrSupportsDefences?result.defences?.evasion:undefined);setEnergyShield(ocrSupportsDefences?result.defences?.energyShield:undefined);setWeaponStats(result.weaponStats??{})
