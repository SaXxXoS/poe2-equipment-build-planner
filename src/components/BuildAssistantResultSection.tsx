  ...
  return <section id="result" className="result build-assistant-result">
    ...
    <details open><summary>Schaden und Build-Vergleich</summary><div className="result-panel damage-estimate">
      {analysis.damageEstimate?.status==='unavailable'?<><h3>Numerischer Schaden: nicht verfügbar</h3><p>{analysis.damageEstimate.warnings.join(' ')}</p></>:<>
        ...
        {analysis.damageEstimate?.enemyProfile ? <><p><b>Vergleichsgegner:</b> {analysis.damageEstimate.enemyProfile.label}. Das Profil wird automatisch aus dem Zielprofil gewählt.</p>{analysis.damageEstimate.enemyProfile.appliedEffects?.length?<div><b>Automatisch berücksichtigte Gegnerwirkungen:</b><ul>{analysis.damageEstimate.enemyProfile.appliedEffects.map(effect=><li key={`${effect.source}:${effect.sourceId}:${effect.kind}`}>{effect.label}: {effect.effectiveValue??effect.value}{effect.kind==='armour-break'?' Rüstung':' %'} · {effect.state==='permanent'?'dauerhaft':effect.state==='fully-active'?'voll aktiv':effect.state==='building'?'wird aufgebaut':'während der Anwendung'}{effect.durationMs?` · ${(effect.durationMs/1000).toLocaleString('de-DE')} s`:''}</li>)}</ul>{analysis.damageEstimate.enemyProfile.hitsToFullyBreakArmour?<p><b>Vollständig gebrochene Rüstung:</b> nach {analysis.damageEstimate.enemyProfile.hitsToFullyBreakArmour} belegten Treffern innerhalb der Wirkzeit.</p>:null}</div>:null}{analysis.damageEstimate.enemyProfile.limitations?.length?<p className="warning"><b>Grenzen:</b> {analysis.damageEstimate.enemyProfile.limitations.join(' ')}</p>:null}</> : <p><b>Vergleichsgegner:</b> Nicht festgelegt; deshalb wird vor Gegnerabwehr gerechnet.</p>}
