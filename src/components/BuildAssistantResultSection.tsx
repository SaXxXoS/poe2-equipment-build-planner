    ...
    <details open><summary>Schaden und Build-Vergleich</summary><div className="result-panel damage-estimate">
      {analysis.damageEstimate?.status==='unavailable'?<><h3>Numerischer Schaden: nicht verfügbar</h3><p>{analysis.damageEstimate.warnings.join(' ')}</p>{analysis.damageEstimate.damageOverTime?.blockedEffects.length?<div><b>Nicht angewandte DoT-Teilwerte:</b><ul>{analysis.damageEstimate.damageOverTime.blockedEffects.map(effect=><li key={`${effect.sourceRecordId}:${effect.kind}`}>{effect.sourceLabel}: {effect.detail}</li>)}</ul></div>:null}</>:<>
        ...
        {analysis.damageEstimate?.resourceSpiritModel?<div><b>Ressourcen und Geist:</b>
          ...
          {analysis.damageEstimate.resourceSpiritModel.spiritReservations.length?<div><p><b>Geistreservierungen:</b></p><ul>{analysis.damageEstimate.resourceSpiritModel.spiritReservations.map(entry=><li key={entry.setupId}><b>{entry.skillName}:</b> {entry.reservationAmount==null?'Betrag unbekannt':`${entry.reservationAmount.toLocaleString('de-DE')} Geist`} · {entry.weaponSet==='set-1'?'Waffenset 1':entry.weaponSet==='set-2'?'Waffenset 2':'beide Waffensets'}</li>)}</ul>{analysis.damageEstimate.resourceSpiritModel.spiritCapacityByWeaponSet.map(state=><p key={state.weaponSet}><b>{state.weaponSet==='set-1'?'Waffenset 1':'Waffenset 2'}:</b> mindestens {state.confirmedMinimumCapacity.toLocaleString('de-DE')} Geist belegt · reserviert {state.reservedSpirit==null?'Unbekannt':state.reservedSpirit.toLocaleString('de-DE')} · verbleibend {state.remainingSpirit==null?'Unbekannt':state.remainingSpirit.toLocaleString('de-DE')} · {state.status==='fits-confirmed-minimum'?'durch die bestätigte Mindestkapazität nutzbar':state.status==='exceeds-confirmed-minimum'?'mit der bestätigten Mindestkapazität noch nicht gedeckt':state.status==='blocked-incomplete-reservation-chain'?'Reservierungskette unvollständig':'keine Reservierung'}</p>)}</div>:null}
