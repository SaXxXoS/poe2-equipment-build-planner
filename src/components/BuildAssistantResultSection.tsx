  ...
  return <section id="result" className="result build-assistant-result">
    ...
    <details open><summary>Schaden und Build-Vergleich</summary><div className="result-panel damage-estimate">
      {analysis.damageEstimate?.status==='unavailable'?<><h3>Numerischer Schaden: nicht verfügbar</h3><p>{analysis.damageEstimate.warnings.join(' ')}</p>{analysis.damageEstimate.damageOverTime?.blockedEffects.length?<div><b>Nicht angewandte DoT-Teilwerte:</b><ul>{analysis.damageEstimate.damageOverTime.blockedEffects.map(effect=><li key={`${effect.sourceRecordId}:${effect.kind}`}>{effect.sourceLabel}: {effect.detail}</li>)}</ul></div>:null}</>:<>
        ...
        {analysis.damageEstimate?.persistentStageState?.skills.length?<div><b>Persistente Stufenzustände:</b><ul>{analysis.damageEstimate.persistentStageState.skills.map(value=><li key={value.skillId}><b>{value.label}:</b> {value.kind==='stationary-retaliation'?`voll vorbereitet ${value.fullStageMinimumAddedColdDamage}–${value.fullStageMaximumAddedColdDamage} zusätzlicher Kälteschaden nach ${((value.fullPreparationTimeMs??0)/1000).toLocaleString('de-DE')} s`:`Vollstufenszenario ${value.fullStageMoreSpellDamagePercent} % mehr Zauberschaden (Faktor ${value.fullStageSpellDamageMultiplier?.toLocaleString('de-DE')})`}. {value.detail}</li>)}</ul><p className="muted">Diese Maximalszenarien werden ohne belegten aktuellen Zustand nicht als dauerhafte DPS eingerechnet.</p></div>:null}
