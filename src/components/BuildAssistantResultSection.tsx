    ...
    <details open><summary>Schaden und Build-Vergleich</summary><div className="result-panel damage-estimate">
      {analysis.damageEstimate?.status==='unavailable'?<><h3>Numerischer Schaden: nicht verfügbar</h3><p>{analysis.damageEstimate.warnings.join(' ')}</p>{analysis.damageEstimate.damageOverTime?.blockedEffects.length?<div><b>Nicht angewandte DoT-Teilwerte:</b><ul>{analysis.damageEstimate.damageOverTime.blockedEffects.map(effect=><li key={`${effect.sourceRecordId}:${effect.kind}`}>{effect.sourceLabel}: {effect.detail}</li>)}</ul></div>:null}</>:<>
        ...
        <dl className="summary-grid">
          ...
          {analysis.damageEstimate?.rageDamageComparison && analysis.damageEstimate.rageDamageComparison.status!=='blocked-no-confirmed-rage-gain' && <div><dt>{analysis.damageEstimate.rageDamageComparison.status==='ramped-sustained-combat-comparison'?'Kampfreferenz bei voller Wut':'Vergleich bei voller Wut'}</dt><dd>{formatDamage(analysis.damageEstimate.rageDamageComparison.expectedDamagePerSecondAfterMitigationAtComparedRage??analysis.damageEstimate.rageDamageComparison.expectedDamagePerSecondAtComparedRage)} Schaden/s bei {analysis.damageEstimate.rageDamageComparison.comparedRage} Wut</dd></div>}
