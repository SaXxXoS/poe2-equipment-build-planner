    ...
    <details open><summary>Schaden und Build-Vergleich</summary><div className="result-panel damage-estimate">
      {analysis.damageEstimate?.status==='unavailable'?<><h3>Numerischer Schaden: nicht verfügbar</h3><p>{analysis.damageEstimate.warnings.join(' ')}</p>{analysis.damageEstimate.damageOverTime?.blockedEffects.length?<div><b>Nicht angewandte DoT-Teilwerte:</b><ul>{analysis.damageEstimate.damageOverTime.blockedEffects.map(effect=><li key={`${effect.sourceRecordId}:${effect.kind}`}>{effect.sourceLabel}: {effect.detail}</li>)}</ul></div>:null}</>:<>
        ...
        {analysis.damageEstimate?.triggerRepeatModel?.sources.length?<div><b>Trigger und Wiederholungen:</b><ul>
          {analysis.damageEstimate.triggerRepeatModel.sources.map(source=><li key={`${source.sourceSkillId}:${source.kind}`}><b>{source.sourceSkillName}:</b> {source.condition?`Bedingung ${source.condition} · `:''}{source.intervalMs?`Intervall ${(source.intervalMs/1000).toLocaleString('de-DE')} s · `:''}{source.targetSkillId?`Ziel ${source.targetSkillId} · `:'Ziel nicht belegt · '}{source.detail}</li>)}
        ...
        {analysis.damageEstimate?.resourceSpiritModel?<div><b>Ressourcen und Geist:</b>
          {analysis.damageEstimate.resourceSpiritModel.equipmentContributions.length?<p>Erfasste Ausrüstungsbeiträge: {analysis.damageEstimate.resourceSpiritModel.equipmentContributions.length}. Sie sind technisch belegt, bilden allein aber noch keinen vollständigen Charakterpool.</p>:<p>Keine numerisch eindeutigen Lebens-, Mana-, Geist- oder Manaregenerationseinträge aus der Ausrüstung erfasst.</p>}
          {analysis.damageEstimate.resourceSpiritModel.confirmedMinimumPools?<p><b>Bestätigter Mindestpool:</b> {analysis.damageEstimate.resourceSpiritModel.confirmedMinimumPools.mana.toLocaleString('de-DE')} Mana · {analysis.damageEstimate.resourceSpiritModel.confirmedMinimumPools.manaRegenerationPerSecond.toLocaleString('de-DE')} Mana pro Sekunde · {analysis.damageEstimate.resourceSpiritModel.confirmedMinimumPools.life.toLocaleString('de-DE')} Leben. Nicht vollständig transportierte zusätzliche Wirkungen sind bewusst nicht eingerechnet.</p>:null}
          {analysis.damageEstimate.resourceSpiritModel.skillCostChains.length?<div><p>Geprüfte Fertigkeitsketten: {analysis.damageEstimate.resourceSpiritModel.skillCostChains.length}. {analysis.damageEstimate.resourceSpiritModel.exactSkillCostsKnown?'Die Grundkosten und alle gewählten Supportmultiplikatoren sind für diese Ketten technisch belegt.':'Mindestens eine Grundkosten- oder Supportmultiplikator-Kette bleibt unbekannt.'}</p><ul>{analysis.damageEstimate.resourceSpiritModel.skillCostChains.flatMap(chain=>chain.baseCosts.map(cost=><li key={`${chain.setupId}:${cost.sourceResource}`}><b>{chain.skillName}:</b> {cost.resourceAdjustedAmount.toLocaleString('de-DE')} {cost.resource==='mana'?'Mana':cost.resource==='mana-percent'?'% Mana':'Raserei'} {cost.cadence==='per-use'?'pro Einsatz':'pro Sekunde'} · Basis {cost.baseAmount.toLocaleString('de-DE')} · Supportmultiplikator {chain.combinedSupportMultiplier==null?'Unbekannt':`${(chain.combinedSupportMultiplier*100).toLocaleString('de-DE')} %`} · Baum/Aszendenz-Kostenfaktor {(chain.combinedResourceCostMultiplier*100).toLocaleString('de-DE')} % · verfügbar {chain.effectiveManaPool==null?'Unbekannt':`${chain.effectiveManaPool.toLocaleString('de-DE')} Mana`} / {chain.effectiveManaRegenerationPerSecond==null?'Unbekannt':`${chain.effectiveManaRegenerationPerSecond.toLocaleString('de-DE')} Mana/s`} · {chain.sustainStatus==='sustainable-on-confirmed-minimum'?'dauerhaft durch den belegten Pool gedeckt':chain.sustainStatus==='burst-affordable-on-confirmed-minimum'?'einsetzbar, dauerhafte Deckung noch nicht belegt':chain.sustainStatus==='unusable-confirmed-zero-mana'?'nicht nutzbar: der vergebene Baum setzt Mana bestätigt auf null':chain.sustainStatus==='blocked-missing-action-frequency'?'Wirkfrequenz unbekannt':chain.sustainStatus==='blocked-missing-character-level'?'Charakterlevel fehlt':'Kostenkette unvollständig'}{chain.manaDemandPerSecond==null?'':` (${chain.manaDemandPerSecond.toLocaleString('de-DE')} Mana/s)`}{chain.confirmedFlatSpiritContribution?` · +${chain.confirmedFlatSpiritContribution.toLocaleString('de-DE')} belegter Geist`:''}{chain.passiveResourceEffects.length?` · ${chain.passiveResourceEffects.length} belegte Baum-/Aszendenzwirkungen`:''}</li>))}</ul></div>:null}
          {analysis.damageEstimate.resourceSpiritModel.spiritReservations.length?<div><p><b>Geistreservierungen:</b></p>{analysis.damageEstimate.resourceSpiritModel.questSpiritEstimate?<p><b>Automatische Quest-Geist-Schätzung:</b> bis zu {analysis.damageEstimate.resourceSpiritModel.questSpiritEstimate.amount.toLocaleString('de-DE')} Geist bei Level {analysis.damageEstimate.resourceSpiritModel.questSpiritEstimate.characterLevel}. Das ist eine Planungsschätzung, kein Nachweis abgeschlossener Quests.</p>:null}<ul>{analysis.damageEstimate.resourceSpiritModel.spiritReservations.map(entry=><li key={entry.setupId}><b>{entry.skillName}:</b> {entry.reservationAmount==null?'Betrag unbekannt':`${entry.reservationAmount.toLocaleString('de-DE')} Geist`} · {entry.weaponSet==='set-1'?'Waffenset 1':entry.weaponSet==='set-2'?'Waffenset 2':'beide Waffensets'}</li>)}</ul>{analysis.damageEstimate.resourceSpiritModel.spiritCapacityByWeaponSet.map(state=><p key={state.weaponSet}><b>{state.weaponSet==='set-1'?'Waffenset 1':'Waffenset 2'}:</b> Planungskapazität {state.planningCapacity.toLocaleString('de-DE')} Geist ({state.confirmedMinimumCapacity.toLocaleString('de-DE')} sicher + {state.levelDerivedQuestSpirit.toLocaleString('de-DE')} levelbasiert) · Grundreservierung {state.reservedSpirit==null?'Unbekannt':state.reservedSpirit.toLocaleString('de-DE')} · nach Effizienz {state.effectiveReservedSpirit==null?'Unbekannt':state.effectiveReservedSpirit.toLocaleString('de-DE')} · verbleibend {state.remainingSpirit==null?'Unbekannt':state.remainingSpirit.toLocaleString('de-DE')} · {state.status==='fits-confirmed-minimum'?'durch die bestätigte Mindestkapazität nutzbar':state.status==='fits-level-derived-quest-estimate'?'unter der Quest-Geist-Schätzung nutzbar':state.status==='exceeds-confirmed-minimum'?'Mindestkapazität überschritten; ohne Level- oder Questangabe unbekannt':state.status==='exceeds-level-derived-quest-estimate'?'auch mit der Planungskapazität nicht gedeckt':state.status==='blocked-incomplete-reservation-chain'?'Reservierungskette unvollständig':'keine Reservierung'}</p>)}</div>:null}
          {analysis.damageEstimate.resourceSpiritModel.sources.length?<ul>{analysis.damageEstimate.resourceSpiritModel.sources.map(source=><li key={`${source.sourceSkillId}:${source.kind}`}><b>{source.sourceSkillName}:</b> {source.kind==='multiple-spirit-reservations'?'mehrere Geistreservierungen':source.kind==='spirit-reservation'?'Geistreservierung':'Manawechselwirkung'} · {source.detail}</li>)}</ul>:<p>Keine strukturierte Reservierungs- oder Ressourcenwechselwirkung der belegten Fertigkeiten gefunden.</p>}
          {analysis.damageEstimate.resourceSpiritModel.semanticSupportCostHints.length?<p>Die ausgewählten Supports besitzen semantische Ressourcenhinweise für das Ranking; sie sind keine exakten Mana- oder Lebenskosten.</p>:null}
          <p className="muted">{analysis.damageEstimate.resourceSpiritModel.limitations.join(' ')}</p>
        </div>:null}
