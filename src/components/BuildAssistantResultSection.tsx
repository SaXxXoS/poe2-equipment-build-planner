  ...
  return <section id="result" className="result build-assistant-result">
    ...
    <details open><summary>Schaden und Build-Vergleich</summary><div className="result-panel damage-estimate">
      {analysis.damageEstimate?.status==='unavailable'?<><h3>Numerischer Schaden: nicht verfügbar</h3><p>{analysis.damageEstimate.warnings.join(' ')}</p>{analysis.damageEstimate.damageOverTime?.blockedEffects.length?<div><b>Nicht angewandte DoT-Teilwerte:</b><ul>{analysis.damageEstimate.damageOverTime.blockedEffects.map(effect=><li key={`${effect.sourceRecordId}:${effect.kind}`}>{effect.sourceLabel}: {effect.detail}</li>)}</ul></div>:null}</>:<>
        ...
        {analysis.damageEstimate?.dualWieldAttackModel?.status==='applied'?<p><b>Beidhändiger Angriff:</b> {analysis.damageEstimate.dualWieldAttackModel.detail} Der angezeigte Trefferwert mittelt Haupt- und Nebenhand; die Aktionsrate verwendet die harmonische mittlere Waffengeschwindigkeit{analysis.damageEstimate.dualWieldAttackModel.hitSequenceMultiplier===2?' und zählt beide Treffer':''}.</p>:null}
        {analysis.damageEstimate?.dualWieldAttackModel&&analysis.damageEstimate.dualWieldAttackModel.status.startsWith('blocked-')?<p className="warning"><b>Beidhändiger Angriff nicht eingerechnet:</b> {analysis.damageEstimate.dualWieldAttackModel.detail}</p>:null}
        ...
        {analysis.damageEstimate?.skillEffectDurationSupportModel?.status==='applied'?<p><b>Wirkungsdauer durch Supports:</b> Faktor {analysis.damageEstimate.skillEffectDurationSupportModel.durationMultiplier.toLocaleString('de-DE',{maximumFractionDigits:3})}. Der Schaden pro Sekunde bleibt unverändert; Dauer und Gesamtschaden einer einzelnen Anwendung ändern sich.</p>:null}
        {analysis.damageEstimate?.skillEffectDurationSupportModel?.status.startsWith('blocked-')?<p className="warning"><b>Dauer-Support nicht angewandt:</b> {analysis.damageEstimate.skillEffectDurationSupportModel.detail}</p>:null}
        {analysis.damageEstimate?.maximumPhysicalDamageSupportModel?.status==='applied'?<p><b>Muskelkraft:</b> {analysis.damageEstimate.maximumPhysicalDamageSupportModel.appliedSupports.map(value=>`${value.finalMaximumPhysicalDamagePercent} % mehr maximaler physischer Schaden`).join(', ')}. Der physische Mindestschaden bleibt unverändert.</p>:null}
        {analysis.damageEstimate?.maximumPhysicalDamageSupportModel?.status.startsWith('blocked-')?<p className="warning"><b>Muskelkraft nicht angewandt:</b> {analysis.damageEstimate.maximumPhysicalDamageSupportModel.detail}</p>:null}
        {analysis.damageEstimate?.areaDamageSupportModel?.status==='applied'?<p><b>Konzentrierte Wirkung:</b> {analysis.damageEstimate.areaDamageSupportModel.appliedSupports.map(value=>`${value.finalAreaDamagePercent} % mehr Flächenschaden`).join(', ')}. Die finale Wirkungsfläche wird mit Faktor {analysis.damageEstimate.areaDamageSupportModel.areaOfEffectMultiplier.toLocaleString('de-DE',{maximumFractionDigits:2})} berechnet.</p>:null}
        {analysis.damageEstimate?.areaDamageSupportModel?.status.startsWith('blocked-')?<p className="warning"><b>Konzentrierte Wirkung nicht angewandt:</b> {analysis.damageEstimate.areaDamageSupportModel.detail}</p>:null}
        {analysis.damageEstimate?.spellCascadeSupportModel?.status==='applied'?<p><b>Zauberkaskade:</b> {analysis.damageEstimate.spellCascadeSupportModel.totalCascadeAreas} Flächen pro Wirkung, Schadensfaktor {analysis.damageEstimate.spellCascadeSupportModel.damageMultiplier.toLocaleString('de-DE',{maximumFractionDigits:2})}, Flächenfaktor {analysis.damageEstimate.spellCascadeSupportModel.areaOfEffectMultiplier.toLocaleString('de-DE',{maximumFractionDigits:2})}. Einzelziel-Überlappung wird ohne Beleg nicht addiert.</p>:null}
        {analysis.damageEstimate?.spellCascadeSupportModel?.status.startsWith('blocked-')?<p className="warning"><b>Zauberkaskade nicht angewandt:</b> {analysis.damageEstimate.spellCascadeSupportModel.detail}</p>:null}
        {analysis.damageEstimate?.chainSupportModel?.status==='applied'?<p><b>Verkettung:</b> {analysis.damageEstimate.chainSupportModel.additionalChains} zusätzliche Verkettung(en), Trefferschadensfaktor {analysis.damageEstimate.chainSupportModel.hitDamageMultiplier.toLocaleString('de-DE',{maximumFractionDigits:2})}. Zusätzliche Zielkontakte werden nicht als Boss-Mehrfachtreffer gerechnet.</p>:null}
        {analysis.damageEstimate?.chainSupportModel?.status.startsWith('blocked-')?<p className="warning"><b>Verkettung nicht angewandt:</b> {analysis.damageEstimate.chainSupportModel.detail}</p>:null}
        {analysis.damageEstimate?.multishotSupportModel?.status==='applied'?<p><b>Mehrfachprojektil:</b> {analysis.damageEstimate.multishotSupportModel.additionalProjectiles} zusätzliche Projektile, Schadensfaktor {analysis.damageEstimate.multishotSupportModel.damageMultiplier.toLocaleString('de-DE',{maximumFractionDigits:2})}, Geschwindigkeitsfaktor {analysis.damageEstimate.multishotSupportModel.skillSpeedMultiplier.toLocaleString('de-DE',{maximumFractionDigits:2})}. Zusätzliche Projektile werden ohne Beleg nicht als Boss-Mehrfachtreffer gerechnet.</p>:null}
        {analysis.damageEstimate?.multishotSupportModel?.status.startsWith('blocked-')?<p className="warning"><b>Mehrfachprojektil nicht angewandt:</b> {analysis.damageEstimate.multishotSupportModel.detail}</p>:null}
        ...
        {analysis.damageEstimate?.conditionalHitEffects?.effects.length?<div><b>Zielabhängige Fertigkeitswirkung:</b><ul>{analysis.damageEstimate.conditionalHitEffects.effects.map(effect=><li key={`${effect.sourceRecordId}:${effect.kind}`}><b>{effect.label}:</b> {effect.conditionValue.toLocaleString('de-DE')} % Schockwirkung ergeben {effect.totalMoreDamagePercent.toLocaleString('de-DE')} % mehr Trefferschaden (Faktor {effect.damageMultiplier.toLocaleString('de-DE',{maximumFractionDigits:3})}). {effect.detail}</li>)}</ul></div>:null}
        {analysis.damageEstimate?.conditionalHitEffects?.blockedEffects.length?<div><b>Nicht angewandte zielabhängige Fertigkeitswirkung:</b><ul>{analysis.damageEstimate.conditionalHitEffects.blockedEffects.map(effect=><li key={`${effect.sourceRecordId}:${effect.kind}`}>{effect.label}: {effect.detail}</li>)}</ul></div>:null}
