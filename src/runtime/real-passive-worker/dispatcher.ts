...
export function createRealPassiveWorkerDispatcher(dependencies:WorkerDispatcherDependencies,send:(message:WorkerToClientMessage)=>void):WorkerDispatcher{
 ...
 async function dispatch(message:ClientToWorkerMessage){
  ...
  if(message.messageType==='dispose'){state='disposed';graph=undefined;prepared=undefined;identity=undefined;sourceVersion=undefined;activeRequestId=undefined;send(envelope(message.requestId,'disposed',{state:'disposed'}));return}
  ...
  if(message.messageType==='analyze'){
   ...
   try{progress(message.requestId,'analysis-progress','validating-request',started);if(message.payload.planning.pointBudget===undefined)throw new Error('missing-point-budget');progress(message.requestId,'analysis-progress','running-orchestrator',started)
    ...
    progress(message.requestId,'analysis-progress','validating-result',started);if(result.realPassivePlanning?.detailMode!=='compact'||result.realPassivePlanning.pipelineResult?.targetingResult)throw new Error('non-compact-worker-result');state='ready';activeRequestId=undefined;progress(message.requestId,'analysis-progress','completed',started);send(envelope(message.requestId,'result',{result,state:'ready'}))
