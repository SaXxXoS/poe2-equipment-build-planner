...
export function adaptImportedPoe2Tree(input: ImportedPoe2Tree, report: TreeImportReport): PassiveTreeViewModel {
  ...
  const classNames = new Map(input.classes.map(value => [value.classStartIndex, value.name.sourceText || String(value.classStartIndex)]))
  ...
  const classes = classStartNodes.map(node => ({ id: node.classId!, displayName: classNames.get(Number(node.classId)) || node.displayName || node.id, nodeId: node.id })).sort((a, b) => a.displayName.localeCompare(b.displayName, 'en'))
  const ascendancies = ascendancyStartNodes.map(node => ({ id: node.ascendancyId || node.id, displayName: ascendancyNames.get(node.ascendancyId || '') || node.displayName || node.id, nodeId: node.id })).sort((a, b) => a.displayName.localeCompare(b.displayName, 'en'))
