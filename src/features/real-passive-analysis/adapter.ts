...
export interface PassiveAnalysisUiInput {character:CharacterConfiguration;equipment:EquipmentEntry[];setups:SkillSetup[];pointBudget:number;weaponSetPointBudget?:number;ascendancyPointBudget?:number;planningMode:PassivePlanningMode;resourcePriority?:'normal'|'undercoverage'}
...
export const realPassiveInitializePayload=():InitializePayload=>({sourceVersion:REAL_PASSIVE_UI_SOURCE_VERSION,expectedTreeIdentity:REAL_PASSIVE_UI_TREE_IDENTITY,expectedPipelineVersion:'5H-1.1.0',expectedContextFormatVersion:'1'})
export function buildRealPassiveWorkerRequest(input:PassiveAnalysisUiInput,requestId:string):PassiveAnalysisAdapterResult{
 ...
 const ascendancyPointBudget=input.ascendancyPointBudget??0
 if(!Number.isInteger(ascendancyPointBudget)||ascendancyPointBudget<0||ascendancyPointBudget>8)errors.push('invalid-ascendancy-point-budget')
 if(ascendancyPointBudget>0&&!input.character.ascendancyId)errors.push('missing-ascendancy-for-points')
