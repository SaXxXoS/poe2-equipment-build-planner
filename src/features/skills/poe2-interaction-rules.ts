...
export function evaluateSupportInteraction(
  ...
  if (support.supportedDamageTypes?.length && !support.supportedDamageTypes.some(tag => skill.tags.includes(tag))) return { status: 'blocked', evidence: 'blocked', score: 0, reason: 'Keine der unterstützten Schadensarten ist auf der Fertigkeit belegt.', ruleId: 'support.damage-type-missing' }
  if (support.supportedMechanics?.length && !support.supportedMechanics.some(tag => skill.tags.includes(tag))) return { status: 'blocked', evidence: 'blocked', score: 0, reason: 'Keine der unterstützten Mechaniken ist auf der Fertigkeit belegt.', ruleId: 'support.mechanic-missing' }
