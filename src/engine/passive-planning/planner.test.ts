...
describe('Planungsmodi und iterative Auswahl',()=>{
 ...
 it('damage-first bevorzugt den belegten höheren Schadenswert',()=>{const damage=planningRecommendation('A',60,{damageScore:140,defenceScore:0});const utility=planningRecommendation('B',120,{damageScore:5,utilityScore:160});expect(planPassiveTargets(passivePlanningInput({targetingResult:planningTargetAnalysis([damage,utility]),planningMode:'damage-first',maximumSelectedTargets:1})).selectedTargetIds[0]).toBe('A')})
