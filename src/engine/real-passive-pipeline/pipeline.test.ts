...
describe('Pipeline-Eingabe und Startknoten',()=>{
 ...
 it('akzeptiert den eigenen Waffenset-Planungsmodus',()=>expect(runRealPassivePipeline(realPassivePipelineInput({planningScope:'weapon-set',pointBudget:2})).violations.map(value=>value.code)).not.toContain('invalid-planning-scope'))
...
describe('Graph, Targeting und Planung',()=>{
 ...
 it('plant Aszendenzpunkte ausschließlich in der gewählten Aszendenz',()=>{const result=runRealPassivePipeline(realPassivePipelineInput({requestId:'ascendancy-fixture',planningScope:'ascendancy',ascendancyId:'foreign',pointBudget:1,candidatePoolLimit:10,maximumSelectedTargets:1}));expect(result).toMatchObject({planningScope:'ascendancy',resolvedStartNodeId:'AS',usedPointBudget:1,remainingPointBudget:0});expect(result.allocatedNodeIds).toEqual(['AS','AX']);expect(result.allocatedNodeIds.every(id=>['AS','AX'].includes(id))).toBe(true)})
 it('blockiert mehr als acht Aszendenzpunkte',()=>expect(runRealPassivePipeline(realPassivePipelineInput({planningScope:'ascendancy',ascendancyId:'foreign',pointBudget:9})).violations.map(value=>value.code)).toContain('ascendancy-point-budget-limit-exceeded'))
...
describe('offizieller Baum Release 0.5.2',()=>{
 ...
 it('hält acht Aszendenzpunkte im offiziellen Baum strikt von normalen Knoten getrennt',()=>{const result=runRealPassivePipeline({...input,requestId:'official-ascendancy',planningScope:'ascendancy',ascendancyId:'Monk3',pointBudget:8,candidatePoolLimit:24,maximumSelectedTargets:8,maximumTargetingResults:100});expect(result.planningScope).toBe('ascendancy');expect(result.usedPointBudget).toBeLessThanOrEqual(8);expect(result.remainingPointBudget).toBe(8-result.usedPointBudget);expect(result.allocatedNodeIds.every(id=>{const node=graph.nodes.get(id);return node?.ascendancyId==='Monk3'&&['ascendancy-start','ascendancy'].includes(node.nodeType)})).toBe(true)},30000)
