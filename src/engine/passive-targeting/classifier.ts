...
const tokenMatch=(text:string,pattern:string)=>{
 let expression=tokenPatternCache.get(pattern)
 if(!expression){
  expression=new RegExp(`(?:^|[^a-z0-9])${pattern.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}(?:$|[^a-z0-9])`,'i')
  tokenPatternCache.set(pattern,expression)
 }
