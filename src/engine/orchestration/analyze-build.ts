...
export function analyzeBuild(request: EngineRequest, context: AnalyzerContext = { engineVersion: ENGINE_VERSION, fixtureMode: true }, modifiers: ModifierDefinition[] = []): BuildAnalysis {
  ...
  const damageEstimate=estimateHitDamage({equipment:request.input.equipment,setups:request.input.skillSetups,skills:request.candidates.skills,fallbackSkillId:selectedRecommendation?.skillId,passiveTree:request.realPassivePlanning?.passiveTree,realPassivePlanning})
