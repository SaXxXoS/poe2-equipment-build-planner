...
export function resolveExecuteSupport(input:{skill:NumericSkill;setup?:SkillSetup;supports:SupportGemDefinition[];enemyProfile?:EnemyMitigationProfile}):ExecuteSupportModel {
  ...
  if(input.enemyProfile?.lifeState==='not-low-life')return empty('inactive-enemy-not-low-life','Das Ziel befindet sich bestätigt nicht auf niedrigem Leben; der Execute-Bonus ist inaktiv.',[],sourceReferences,blocksPlayerLowLife)
