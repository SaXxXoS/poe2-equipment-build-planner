...
export interface IntrinsicSkillCostEffect { statId:string;kind:'cost-increased';value:number;evidence:'structured-exact';sourceReference:string }
export interface BlockedIntrinsicSkillCostEffect { statId:string;value:number;reason:'requires-runtime-spend-rate'|'requires-max-mana-and-target-skill-chain'|'requires-channel-duration-state';sourceReference:string }
