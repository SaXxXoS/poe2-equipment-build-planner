...
export interface ProjectileHitMechanic { kind:'projectiles-per-action'|'chain-count'|'pierce-count'|'pierce-chance'|'fork-enabled'|'terrain-chain-chance'|'maximum-hit-cap';value:number;sourceReference:string;evidence:'structured-exact';damageUse:'coverage-only'|'blocked-as-damage-multiplier';detail:string }
...
export interface DamageEstimate {
  ...
  rigwaldFerocitySupportModel?:{
    ...
    damageMultiplier:number
    attackSpeedMultiplier:number
    appliedSupports:Array<{supportId:string;supportName:string;family:string;finalDamagePercent:number;attackSpeedPercent:number;sourceReferences:string[]}>
    blockedSupportIds:string[]
    sourceReferences:string[]
    detail:string
  }
  controlledDestructionSupportModel?:{
    ...
    appliedSupports:Array<{supportId:string;supportName:string;family:string;hitDamagePercent:number;sourceReferences:string[]}>
    blockedSupportIds:string[]
    sourceReferences:string[]
    detail:string
  }
