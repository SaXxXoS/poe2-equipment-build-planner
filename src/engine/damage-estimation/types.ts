...
export interface IntrinsicSkillCostEffect { statId:string;kind:'cost-increased'|'archmage-max-mana-cost';value:number;additionalBaseManaCost?:number;gainAsLightningPercent?:number;sourceSkillId?:string;evidence:'structured-exact';sourceReference:string }
export interface BlockedIntrinsicSkillCostEffect { statId:string;value:number;reason:'requires-runtime-spend-rate'|'requires-max-mana-and-target-skill-chain'|'requires-confirmed-maximum-mana'|'requires-channel-duration-state';sourceReference:string }
