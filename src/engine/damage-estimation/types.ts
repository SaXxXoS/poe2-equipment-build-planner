...
export interface AppliedDamagingAilmentEffect { sourceRecordId:string;sourceLabel:string;kind:'bleeding'|'poison';damageType:'physical'|'chaos';status:'sustained-exact-input-chain';chancePercent:number;durationMs:number;maximumStacks:number;expectedActiveStacks:number;damagePerSecond:number;totalDamagePerApplication:number;effectMultiplier:number;detail:string }
...
export interface DamageEstimate {
  ...
  damagingAilments?:{modelVersion:string;effects:AppliedDamagingAilmentEffect[];blockedEffects:BlockedDamagingAilmentEffect[];totalSustainedDamagePerSecond?:number;limitations:string[]}
