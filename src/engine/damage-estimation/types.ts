...
export interface ProjectileHitMechanic { kind:'projectiles-per-action'|'chain-count'|'pierce-count'|'pierce-chance'|'fork-enabled'|'maximum-hit-cap';value:number;sourceReference:string;evidence:'structured-exact';damageUse:'coverage-only'|'blocked-as-damage-multiplier';detail:string }
...
export interface DamageEstimate {
  ...
  crossbowAmmunitionSupportModel?:{
    modelVersion:string
  ...
  pierceSupportModel?:{
    ...
    singleTargetHitMultiplier:1
    appliedSupports:Array<{supportId:string;supportName:string;family:string;chanceToPiercePercent:number;finalDamageAfterPiercingPercent:number;sourceReferences:string[]}>
    blockedSupportIds:string[]
    sourceReferences:string[]
    limitations:string[]
    detail:string
  ...
  projectileHitModel?:{modelVersion:string;isProjectileSkill:boolean;projectilesPerAction:number;singleTargetHitMultiplier:1;mappingPotentialTargetContacts:number;supportPierceChancePercent:number;postPierceDamageMultiplier:number;forkEnabled:boolean;forkedProjectileDamageMultiplier:number;mechanics:ProjectileHitMechanic[];bossScenario:{hitMultiplier:1;status:'single-hit-only';detail:string};mappingScenario:{potentialTargetContacts:number;status:'coverage-estimate';detail:string};limitations:string[]}
