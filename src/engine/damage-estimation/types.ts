...
export interface TriggerRepeatSource { sourceSkillId:string;sourceSkillName:string;kind:'meta-trigger'|'inbuilt-trigger'|'repeat-interval';condition?:string;intervalMs?:number;targetSkillId?:string;energyRequirement?:number;baseEnergyPerEvent?:number;energyGenerationModifierPercent?:number;effectiveEnergyPerEventAtMonsterPowerOne?:number;eventsRequiredAtMonsterPowerOne?:number;eventRatePerSecond?:number;energyPerSecondAtMonsterPowerOne?:number;triggerRatePerSecondAtMonsterPowerOne?:number;secondsPerTriggerAtMonsterPowerOne?:number;targetDamageMultiplier?:number;targetExpectedHitDamage?:number;targetExpectedHitDamageAfterMitigation?:number;normalizedTriggeredDamagePerSecondAtMonsterPowerOne?:number;normalizedTriggeredDamagePerSecondAfterMitigationAtMonsterPowerOne?:number;status:'blocked-missing-target'|'blocked-incompatible-target'|'blocked-missing-trigger-source'|'blocked-missing-interval'|'normalized-event-rate-only'|'normalized-target-damage-only'|'interval-only';evidence:'structured-exact';sourceReferences:string[];detail:string }
...
export interface SkillResourceCostChain { setupId:string;skillId:string;skillName:string;weaponSet:'set-1'|'set-2'|'both';selectedSupportIds:string[];semanticSupportCostHints:number[];baseCosts:SkillResourceCostValue[];supportCostMultipliers:AppliedSupportCostMultiplier[];passiveResourceEffects:PassiveResourceEffect[];combinedSupportMultiplier:number|null;combinedResourceCostMultiplier:number;effectiveManaPool:number|null;effectiveManaRegenerationPerSecond:number|null;confirmedFlatSpiritContribution:number;baseCostStatus:'structured-exact-level'|'structured-exact-zero-cost'|'blocked-missing-exact-base-cost';supportMultiplierStatus:'structured-exact-all-selected-supports'|'structured-exact-no-supports'|'blocked-missing-exact-support-cost-multipliers';poolStatus:'confirmed-minimum-pool'|'confirmed-pool-with-passive-effects'|'blocked-missing-character-level';sustainStatus:'sustainable-on-confirmed-minimum'|'burst-affordable-on-confirmed-minimum'|'unusable-confirmed-zero-mana'|'blocked-missing-action-frequency'|'blocked-missing-character-level'|'blocked-missing-exact-cost-chain';actionFrequencyPerSecond:number|null;manaDemandPerSecond:number|null }
